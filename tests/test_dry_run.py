...
def test_write_dry_run_output_creates_md_and_html(tmp_path, monkeypatch):
    ...
    md_path, html_path = main.write_dry_run_output(content, "en")
...
def test_dry_run_filename_uses_language(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    md_path, html_path = main.write_dry_run_output("body", "zh")
    assert md_path.name == "digest_zh.md"
    assert html_path.name == "digest_zh.html"
